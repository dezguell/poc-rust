...
const CHECK_CONDITION_TO_CONTINUE: &u32 = &u32::MIN;
...
fn main() {      
    ...
    loop{        
        ...
        if check_hint_request(&guess_str){
            ...
            guess_int = convert_str_to_int(&mut guess_str);
            
            if guess_int.eq(CHECK_CONDITION_TO_CONTINUE){
                continue;
            }  
...
fn convert_str_to_int(_text: &mut std::string::String) -> u32 {
    if _text.eq(&"0"){
        return *CHECK_CONDITION_TO_CONTINUE;
    }    
    match _text.trim().parse::<u32>()
            {
                Ok(num) => num,
                Err(_) => *CHECK_CONDITION_TO_CONTINUE,                
            }    
}
